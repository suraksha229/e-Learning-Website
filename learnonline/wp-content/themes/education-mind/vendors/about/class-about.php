...
if ( ! class_exists( 'Education_Mind_About' ) ) {
	...
	class Education_Mind_About {
		...
		/**
		 * Singleton instance of Education_Mind_About.
		 *
		 * @var Education_Mind_About $instance Education_Mind_About instance.
		 */
		private static $instance;

		/**
		 * Main Education_Mind_About instance.
		 *
		 * @since 1.0.0
		 *
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Education_Mind_About ) ) {
				self::$instance = new Education_Mind_About;
		...
		public function setup_config() {
			...
			$dismiss_url = wp_nonce_url( add_query_arg( array( 'education-mind-at-dismiss' => '1' ), admin_url( 'themes.php' ) ), 'education-mind-action-dismiss-' . get_current_user_id(), 'education-mind-dismiss-nonce' );
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.', 'education-mind' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '" class="button button-primary" style="text-decoration: none;">' . sprintf( esc_html__( 'Get started with %s', 'education-mind' ), $this->theme_name ) . '</a>' ) . '<a href="' . esc_url( $dismiss_url ) . '" class="btn-at-dismiss">' . esc_html__( 'Dismiss', 'education-mind' ) . '</a></p>';
		}
		...
		public function setup_actions() {
			...
			add_action( 'wp_ajax_nopriv_at_about_action_dismiss_recommended_action', array( $this, 'dismiss_recommended_action_callback' ) );
		}
		...
		function hide_notice() {
			// Verify nonce.
			if (! ( isset( $_GET['education-mind-dismiss-nonce'] ) && wp_verify_nonce( sanitize_key( $_GET['education-mind-dismiss-nonce'] ), 'education-mind-action-dismiss-' . get_current_user_id() ) ) ) {
				return;
			}

			if ( 1 === absint( $_GET['education-mind-at-dismiss'] ) ) {
				update_user_meta( get_current_user_id(), 'education_mind_at_dismissed', 1 );
			}
		}
		...
		public function render_about_page() {
			...
			if ( ! empty( $welcome_title ) || ! empty( $welcome_content ) || ! empty( $this->tabs ) ) {
				...
				echo '<div class="wrap about-wrap at-wrap">';
		...
		public function activation_admin_notice() {
			...
			$dismissed_status = get_user_meta( get_current_user_id(), 'education_mind_at_dismissed', true );
		...
		public function load_assets( $hook ) {
			...
			if ( 'appearance_page_' . $this->page_slug === $hook ) {
				...
				wp_enqueue_style( 'education-mind-about', get_template_directory_uri() . '/vendors/about/css/about' . $min . '.css', array(), '1.0.1' );
				wp_enqueue_script( 'education-mind-about', get_template_directory_uri() . '/vendors/about/js/about' . $min . '.js', array( 'jquery' ), '1.0.1' );
				$js_vars = array(
					'ajaxurl' => esc_url( admin_url( 'admin-ajax.php' ) ),
				);
				wp_localize_script( 'education-mind-about', 'educationMindAboutObject', $js_vars );
			}
		}
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							switch ( $active['needs'] ) {
								...
								case 'install':
									...
									$label = esc_html__( 'Install', 'education-mind' );
									break;
								case 'activate':
									$class = 'activate-now button button-primary';
									$label = esc_html__( 'Activate', 'education-mind' );
									break;
								case 'deactivate':
									$class = 'deactivate-now button';
									$label = esc_html__( 'Deactivate', 'education-mind' );
									break;
							}
							?>
		...
		public function upgrade_to_pro() {
			...
			if ( isset( $upgrade_to_pro['button_link'] ) && ! empty( $upgrade_to_pro['button_link'] ) ) {
				$button_text = esc_html__( 'Upgrade to Pro', 'education-mind' );
		...
		public function useful_plugins() {
			...
			if ( isset( $tgmpa ) ) {
				...
				if ( true === $complete ) {
					echo '<div class="all-active"><span class="dashicons dashicons-yes"></span>&nbsp;' . esc_html__( 'Great, all recommended plugins are intalled and active.', 'education-mind' ) . '</div>';
				}
				...
				if ( true !== $complete ) {
					echo '<div class="button-wrap"><a href="' . esc_url( $url ) . '" class="button button-primary">' . esc_html__( 'Manage Plugins', 'education-mind' ) . '</a></div>';
				}
