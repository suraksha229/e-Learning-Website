=== Education Care ===

Contributors: preciousthemes
Tags: custom-background, custom-colors, custom-header, custom-menu, featured-image-header, featured-images, flexible-header, full-width-template, one-column, two-columns, sticky-post, theme-options, threaded-comments, translation-ready, footer-widgets, blog, education, entertainment

Requires at least: 4.5
Tested up to: 4.8.3
Stable tag: 1.0.8
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html


== Description ==

Education Care is free, full responsive, clean, modern looking and professional WordPress theme. This theme is best for kindergarten, school, university, college, academic institutions, training centers, coaches, LMS and other educational websites. It is not bound to educational websites as it is multipurpose theme. It can be used for any general, minimal, multipurpose, business, corporate, portfolio, digital agency, landing page, product showcase and all informative websites. Education care comes with inbuilt features like customizer to change all settings of website, widget section which can be managed by drag and drop, gallery, call to action, feature highlights, stat counter, featured courses and more. Speed optimization, security, dedicated support forum and well documentation are major features of this theme. Education Care aims to provide you best and unique website with minimal technical knowledge. 

== Installation ==

1. In your admin panel, go to Appearance > Themes and click the Add New button.
2. Click Upload and Choose File, then select the theme's .zip file. Click Install Now.
3. Click Activate to use your new theme right away.

== Frequently Asked Questions ==

= Does this theme support any plugins? =

Education Care includes support for Infinite Scroll in Jetpack.

== License ==

Education Care WordPress Theme, Copyright (C) 2017, Precious Themes
Education Care is distributed under the terms of the GNU GPL

This theme support only one level menu at footer.

== Changelog ==

= 1.0.8 =
* Full width template added

= 1.0.7 =
* Fixed fatal errors on PHP 5.2
* Pager navigation added for testimonial slider

= 1.0.6 =
* Minor CSS updates

= 1.0.5 =
* Ordered and Unordered list issue solved
* Go to top button added

= 1.0.4 =
* Minor CSS change for overlay of menu and banner
* Change in header layout
* Sub menu style changed
* Slider style updated
* Screenshot changed


= 1.0.3 =
* Responsive menu issues fixed
* Missing info of third party script added
* Header image and background image issue solved

= 1.0.2 =
* Escape function added for missing places
* Unused js and css files removed
* Design issues fixed

= 1.0.1 =
* Courses Widget Added
* News ticker added
* Responsive menu updated
* Testimonial layout updated

= 1.0.0 =
* Initial release

== Credits ==

- Underscores
Source: Based on Underscores http://underscores.me/, (C) 2012-2015 Automattic, Inc. 
License: [GPLv2 or later](https://www.gnu.org/licenses/gpl-2.0.html)

- Normalize
Source: normalize.css http://necolas.github.io/normalize.css/, (C) 2012-2015 Nicolas Gallagher and Jonathan Neal
License: [MIT](http://opensource.org/licenses/MIT)

- Google Fonts
Source: https://fonts.google.com/specimen/Poppins
License: SIL Open Font License, 1.1 - scripts.sil.org/OFL

- Font Awesome: 
Source: http://fontawesome.io/, (C) Dave Gandy 
License: (CSS)[MIT](http://opensource.org/licenses/MIT), (Fonts)[SIL OFL 1.1](http://scripts.sil.org/OFL)

- lightGallery
Source: http://sachinchoolur.github.io/lightGallery/
License: GPLv3 (https://www.gnu.org/licenses/gpl-3.0.html)

- Breadcrumb Trail 
Source: http://themehybrid.com/plugins/breadcrumb-trail/, (C) 2008 - 2015, Justin Tadlock
License: [GPLv2](https://www.gnu.org/licenses/gpl-2.0.html)

- Counterup
Source: Benjamin Intal http://gambit.ph @bfintal
License: [GPLv2 or later](https://www.gnu.org/licenses/gpl-2.0.html)

- Waypoints 
Source: (C) 2011-2016 Caleb Troughton (https://github.com/imakewebthings/waypoints/blob/master/licenses.txt)
License: [MIT](http://opensource.org/licenses/MIT)

- Waypoints 
Source: (C) 2011-2016 Caleb Troughton (https://github.com/imakewebthings/waypoints/blob/master/licenses.txt)
License: [MIT](http://opensource.org/licenses/MIT)

- Slick
Source: (C) 2013-2016 Ken Wheeler (https://github.com/kenwheeler/slick/blob/master/LICENSE)
License: [MIT](http://opensource.org/licenses/MIT)

== Images ==
- Images used in screenshot, License CC0 Public Domain, (https://pixabay.com/en/graduation-graduation-day-2038866/ and https://www.pexels.com/photo/woman-wearing-red-handkerchief-on-neck-holding-black-microphone-159671/)